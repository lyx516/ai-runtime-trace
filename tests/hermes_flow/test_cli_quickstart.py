...
def test_cli_init_dry_run_no_runtime(sample_flow_yaml_path: Path, tmp_project_root: Path) -> None:
    """CLI init with --dry-run must not create any .hermes-flow/runs/ directory."""
    result = subprocess.run(
    ...
    data = json.loads(result.stdout)
    assert data.get("ok") is True
    runs_dir = tmp_project_root / ".hermes-flow" / "runs"
    assert not runs_dir.exists(), "dry-run should not create runtime state"

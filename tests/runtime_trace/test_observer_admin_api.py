...
from runtime_trace.observer import SSEHandler
...
def test_observer_lists_runs_from_all_scanned_dirs(tmp_path, monkeypatch):
    """Observer now scans fixed, env-override, and legacy (.runtime-trace/runs) dirs."""
    fixed_runs = tmp_path / "fixed-runs"
    legacy_runs = tmp_path / "project" / ".runtime-trace" / "runs"
    # Simulate an env-override dir (like agent-pool's RUNTIME_TRACE_RUNS_DIR)
    pool_runs = tmp_path / "agent-pool" / ".runtime-trace" / "runs"
    ...
    monkeypatch.setenv("RUNTIME_TRACE_RUNS_DIR", str(pool_runs))

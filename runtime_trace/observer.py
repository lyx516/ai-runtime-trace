"""Observer — real-time observability for Runtime Trace runs.
...
Usage:
    from runtime_trace.observer import FlowObserver
    observer = FlowObserver(port=8080)
    observer.start()  # background thread

    # Or standalone:
    python -m runtime_trace.observer --port 8080 --project-root /path/to/project
"""
...
from runtime_trace.run_paths import get_runs_dir
...
def _find_runs_dir(project_root: str | None = None) -> Path:
    """Return the canonical .runtime-trace/runs directory."""
...
class SSEHandler(http.server.BaseHTTPRequestHandler):
    ...
    def _iter_runs_dirs(self) -> list[Path]:
        ...
        dirs = []
        # 1. Env override (RUNTIME_TRACE_RUNS_DIR)
        _env_runs = os.environ.get("RUNTIME_TRACE_RUNS_DIR")
        if _env_runs:
            _p = Path(_env_runs).expanduser().resolve()
            if _p.exists():
                dirs.append(_p)
        # 2. Primary: default  (runtime_trace/.runtime-trace/runs)
        if self.runs_dir.exists():
            dirs.append(self.runs_dir)
        # 3. Fallback: git root .runtime-trace/runs
        if self.project_root:
            _legacy = Path(self.project_root) / ".runtime-trace" / "runs"
    ...
    def do_POST(self):
        ...
        if self.path == "/api/resume":
            ...
            try:
                from runtime_trace.tools import flow_resume
    ...
    def _get_agent_pool_dir(self) -> Path | None:
        ...
        candidates = [
            ...
            Path.home() / ".runtime-trace" / "plugins" / "agent-pool",
        ]
    ...
    def _serve_pool_api(self):
        ...
        return self._send_json({"agents": agents_list, "pool_id": "runtime-trace-agent-pool"})
...
class FlowObserver:
    """Real-time observer for Runtime Trace runs.
...
def ensure_observer(port: int = 8765, project_root: str | None = None) -> FlowObserver:
    ...
    # Honour RUNTIME_TRACE_RUNS_DIR if set (so observer finds runs alongside auto-debate.py)
    _runs_dir = os.environ.get("RUNTIME_TRACE_RUNS_DIR")
    if _runs_dir:
...
<title>Runtime Trace — State Flow</title>
<style>
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Runtime Trace Observer")
    ...
    print(f"Runtime Trace Observer starting on http://localhost:{args.port}")

...
SIX CALLING SHAPES (inferred from query parameter):
  ...
  BASELINE — agent_recall(query="baseline", goal_kw="词频"): cross-run average tool_calls from run_performance table for similar tasks.

PAGINATION: when has_more=true, use offset + limit for next page. Use to investigate agent behavior BEFORE making judgments.""",
    "parameters": {
        "type": "object",
        "properties": {
            "query": {"type": "string", "enum": ["overview", "transitions", "decisions", "thinking", "messages", "baseline"], "description": "What data to recall."},
            ...
            "goal_kw": {"type": "string", "description": "Keyword to match run goals for baseline queries (e.g. '词频', 'Shell')."},

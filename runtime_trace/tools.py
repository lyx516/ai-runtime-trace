...
from runtime_trace.engine import advance_state, detect_idle_timeout, evaluate_gate
from runtime_trace.errors import RuntimeStateError
from runtime_trace.flow_loader import load_flow_from_yaml, validate_flow
from runtime_trace.run_paths import get_run_dir
from runtime_trace.routing import validate_message
from runtime_trace.schemas import (
    AgentBinding,
    Decision,
    DeliveryOutcome,
    FlowInitResult,
...
from runtime_trace.trace import SqliteTracer, set_tracer, get_tracer
...
def _get_store(run_id: str) -> RuntimeStore:
    ...
    for base in [Path.cwd(), Path.home()]:
        matches = list(Path(base / ".runtime-trace" / "runs").glob(f"{run_id}*"))
        if matches:
